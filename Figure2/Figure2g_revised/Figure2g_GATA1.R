...
DiffMap <- join(DiffMap, scores[,c("Cell", "GATA1")], by="Cell", type="left")

DiffMap <- DiffMap[order(DiffMap$GATA1), ]
...
ggplot(DiffMap, aes(x=-DC1, y=-DC2, z=-DC4, color=GATA1)) +
  geom_point(DiffMap, mapping=aes(x=-DC1, y=-DC2, z=-DC4, color=GATA1))+
  theme_void() +
  axes_3D() +
...
ggsave(filename="Figure2g_GATA1.png", width=8, height=8)
