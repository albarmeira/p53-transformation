...
#Mapping vangalen dataset into Greenleaf's LSI projection
#LSI integration
...
#Read in Summarized Experiment from vanGalen dataset baseline timepoints (generated in 02_prepare_SummExp.R)
seDisease <- readRDS("../../../Sources/LSI/vangalen_baseline.rds")

#Subset SE by Disease Cells
id<- "malignant"
seDisease <- seDisease[,grep(id, colData(seDisease)$PredictionRefined)]
seDisease$dataset<-c('vanGalen')
...
umapManifold <- uwot::load_uwot("../../../Sources/LSI/scRNA-Healthy-Hematopoiesis-UMAP-model.uwot")
...
projectionDF$color[projectionDF$Type!="reference"]<-"AML"
...
p+stat_density_2d(data=projectionDF_NOTreference,aes(X1,-X2,color=color),size=1)+
  scale_color_manual(values=c("darkgreen","lightgrey"))

#ggsave('Figure2d_Panel2.png',height=6,width=6)
