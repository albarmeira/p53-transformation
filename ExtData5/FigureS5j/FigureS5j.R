#FigureS5j - Greenleaf-derived myeloid signatures from TCGA dataset
...
df <- df[which(!is.na(df$myeloid.greenleaf.score)), ]
dim(df) #151 patients
...
# Boxplot
    ...
    # Save file
    ggsave(file="FigureS5j.png", width=2, height=4)

# Statistical test
wilcox.test(y~x)$p.value #0.5499375
