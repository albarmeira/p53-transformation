...
#Subset IF0318 donor
metadata<-subset(metadata,donor=="IF0318")
...
ggplot(metadata,aes(y=CD38+CD38_pseudovalue,x=CD34,fill=cell_type))+
  geom_point(size=5,shape=21,stroke=1) + theme_bw()+
  scale_x_continuous(limits = c(1,120000))+
  scale_y_log10(limits = c(1000,500000))+
  geom_hline(yintercept = 6000+CD38_pseudovalue)

ggplot(metadata,aes(y=CD38+CD38_pseudovalue,x=CD34,fill=genotype.classification))+
...
ggsave('FigureS3a_IF0318_CD38neg.png',height=5,width=5)
...
ggplot(CD38pos,aes(y=CD123+CD123_pseudovalue,x=CD45RA,fill=genotype.classification))+
  ...
  stat_density_2d(color="red3",size=1) +
  geom_point(size=3,shape=21,stroke=1,alpha=0.2) +
  scale_y_log10(limits=c(300,50000))+
  scale_x_log10(limits=c(800,50000))+
  geom_hline(yintercept = 1000+CD123_pseudovalue)+
  geom_vline(xintercept = 5000)+
  theme(panel.grid.major.x = element_blank(),panel.grid.minor.x=element_blank())+
  theme(panel.grid.major.y = element_blank(),panel.grid.minor.y=element_blank()) +
  theme(panel.border = element_rect(size=4,colour = "black"))+
...
ggsave('FigureS3a_IF0318_CD38pos.png',height=5,width=5)
